...
export const listNotes = () => async (dispach, getState) => {
    ...
    try {
        ...
        if (data) {
            dispach({
                type: NOTE_LIST_SUCCESS, payload: data
            })
        }
...
export const createNote = (title, content, category) => async (dispach, getState) => {
    const API_URL = '/api/notes'
    ...
    try {
        ...
        const { data } = await axios.post(API_URL + "/create", { title, content, category }, config);
...
export const updateNote = (id, title, content, category) => async (dispatch, getState) => {
    ...
    try {
        ...
        const { data } = await axios.put(API_URL + `/${id}`, { title, content, category }, config)
